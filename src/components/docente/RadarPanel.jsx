...
export default function RadarPanel({ cursoRows, programaRows, shortCriteriaLabels, criteriaLabels, nCrit, onOpenCriteriaInfo, view, onViewChange }) {

  const { config, docenteAvgs, programaAvgs } = useMemo(() => {
    const docenteAvgs = computeCriteriaAverages(cursoRows, nCrit);
    const programaAvgs = computeCriteriaAverages(programaRows, nCrit);
    return { config: radarConfig(shortCriteriaLabels, docenteAvgs, programaAvgs), docenteAvgs, programaAvgs };
    ...
    >
      ...
      {hasData && view === 'barras' && (
        <div className={styles.criteriaBreakdown}>
          {shortCriteriaLabels.map((short, i) => {
            ...
            return (
              <div key={short} className={styles.criteriaBreakdownRow}>
                <div className={styles.criteriaBreakdownHeader}>
                  <span className={styles.criteriaBreakdownShort}>{short}</span>
                  <span className={styles.criteriaBreakdownFull} title={criteriaLabels[i]}>{criteriaLabels[i]}</span>
