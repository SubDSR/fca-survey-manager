import { computeDescriptiveStats, computeDirectiveCounts } from '../../lib/stats.js';
...
function PrintCriteriaDetail({ cursoRows, programaAvgs, criteriaLabels }) {
  ...
  const programaAvgsSafe = programaAvgs || [];
      ...
      <table>
        ...
        <tbody>
          {criteriaLabels.map((label, i) => {
            ...
            const progAvg = Math.round((programaAvgsSafe[i] || 0) * 10) / 10;
...
export default function PrintStatBox({ cursoRows, programaAvgs, criteriaLabels }) {
  return (
    <>
      <PrintDescriptiveStats cursoRows={cursoRows} />
      <PrintCriteriaDetail cursoRows={cursoRows} programaAvgs={programaAvgs} criteriaLabels={criteriaLabels} />
    </>
  );
}
