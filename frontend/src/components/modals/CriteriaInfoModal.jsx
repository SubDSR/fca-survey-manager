...
/* Portado desde reference/dashboard_evaluacion_docente.html:
   ...
   `cursoRows` es el payload que envían DocenteView.jsx/CursoView.jsx al abrir
   este modal: NO son las filas de consolidado (esas ya no traen scores por
   encuestado), sino las filas "detalladas" ya construidas ahí mismo desde
   GET /api/encuestas/respuestas (ver lib/rawResponsesFromView.js#buildDetailedResponseRows),
   con el mismo shape que antes traía dataset.csv (scores/notaFinal por
   encuestado). El promedio del programa (columna "Prom. programa"), en cambio,
   sólo necesita un promedio -- no rango ni N° -- así que se calcula desde la
   vista de criterios (v_promedio_por_criterio, GET /api/encuestas/criterios,
   ya en contexto) filtrada a los grupos del programa, igual que RadarPanel.jsx. */
...
export default function CriteriaInfoModal({ open, onClose, cursoRows }) {
  const { groupRows, criterios, criteriaLabels } = useData();
  ...
  const detail = useMemo(() => {
    ...
    const groupRowsPrograma = groupRows.filter((r) => r.programa === first.programa);
    const programaAvgs = computeCriteriaAveragesFromView(criterios, groupRowsPrograma).avgs;
  ...
  }, [first, rowsDocenteCurso, groupRows, criterios, criteriaLabels]);
