...
/* Portado desde reference/dashboard_evaluacion_docente.html: renderCicloTrendChart (líneas 1687-1764).
   `rows` son en realidad grupos ya agregados (ver lib/directorGroups.js): se
   pondera por `n` en vez de promediar respuestas crudas. */
...
export default function CicloTrendChart({ rows }) {
  const rowsByCiclo = useMemo(() => {
    ...
    return ciclos.map((ciclo) => {
      ...
      const totalN = cicloRows.reduce((a, r) => a + r.n, 0);
      const nota = totalN
        ? Math.round((cicloRows.reduce((a, r) => a + r.nota * r.n, 0) / totalN) * 10) / 10
        : 0;
      const cumplimiento = totalN
        ? Math.round(cicloRows.reduce((a, r) => a + r.cumplimiento * r.n, 0) / totalN)
        : 0;
