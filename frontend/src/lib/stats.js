...
// Promedio ponderado por r.n (número de encuestas de cada grupo docente+
// asignatura+ciclo+sección). Necesario porque `rows` ya no es una fila por
// encuesta individual (dataset.csv) sino una fila por grupo, agregada en el
// backend (GET /api/encuestas/consolidado, ver lib/directorGroups.js) — un
// promedio simple de r.notaFinal pesaría igual una sección de 3 encuestas
// que una de 30.
export function weightedNotaPromedio(rows) {
...
export function classifyDocentesByEstado(rows, umbralAprobacion) {
  ...
  rows.forEach((r) => {
    ...
    byDocente.get(r.docente).push(r);
  });
